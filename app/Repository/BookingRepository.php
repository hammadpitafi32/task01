...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($userId)
    {
        ...
        if (!$currentUser) {
            return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => $normalJobs, 'currentUser' => null, 'userType' => $userType];
        }
        ...
        if ($currentUser->is('customer')) {
            ...
            $userType = 'translator';
            $jobs = Job::getTranslatorJobs($currentUser->id, 'new')->pluck('jobs')->flatten();
        } else {
            $jobs = collect(); // Handle cases where user is neither customer nor translator
        }
        ...
        // Sort normal jobs after processing all
        $sortedNormalJobs = collect($normalJobs)->sortBy('due');

        return [
            'emergencyJobs' => $emergencyJobs,
            'normalJobs' => $sortedNormalJobs->values()->all(), // Use values() to reset keys after sorting
    ...
    {
        // Directly use Laravel's provided methods for cleaner, more idiomatic pagination handling
        $pageNumber = $request->input('page', 1); // Default to page 1 if none specified
        $perPage = 15; // Items per page could be made configurable via request or a constant
        ...
        $userType = $currentUser->type; // Assuming there's a type attribute. Adjust according to your model.

        if ($currentUser->is('customer')) {
            // Paginate completed jobs directly
            ...
            // Assuming getTranslatorJobsHistoric already supports pagination
            $jobs = Job::getTranslatorJobsHistoric($currentUser->id, 'historic', $pageNumber);
        } else {
            // Handling for users who are neither customers nor translators
            return response()->json(['message' => 'No applicable jobs found for user type'], 404);
        }
    ...
    public function store($user, $data)
    {
        ...
        // Check if the user is allowed to create a booking
        if ($user->user_type != env('CUSTOMER_ROLE_ID')) {
            return ['status' => 'fail', 'message' => 'Translator cannot create bookings'];
        }

        // Set the default responses and data attributes
        $jobAttributes = $this->prepareJobAttributes($user, $validatedData);

        // Create the job
        $job = $user->jobs()->create($jobAttributes);

        // Dispatch job created event (e.g., for notifications)
        event(new JobWasCreated($job, $validatedData));
    ...
    protected function prepareJobAttributes($user, $data)
    {
        $immediate = $data['immediate'] === 'yes';
        $attributes = [
            'immediate' => $immediate ? 'yes' : 'no',
            'due' => $immediate ? Carbon::now()->addMinutes(5)->format('Y-m-d H:i:s') : $this->formatDueDate($data),
            'customer_phone_type' => $data['customer_phone_type'] ?? 'no',
            'customer_physical_type' => $data['customer_physical_type'] ?? 'no',
            'certified' => $this->determineCertification($data['job_for']),
            'gender' => $this->determineGender($data['job_for']),
            'job_type' => $user->userMeta->consumer_type === 'rwsconsumer' ? 'rws' :
                         ($user->userMeta->consumer_type === 'ngo' ? 'unpaid' : 'paid'),
            'b_created_at' => now()->format('Y-m-d H:i:s'),
            'will_expire_at' => isset($data['due_date']) ? TeHelper::willExpireAt($data['due_date'], now()) : null,
    ...
    protected function formatDueDate($data)
    {
        $due = $data['due_date'] . ' ' . $data['due_time'];
        return Carbon::createFromFormat('m/d/Y H:i', $due)->format('Y-m-d H:i:s');
    }

    protected function determineCertification(array $jobFor)
    {
    ...
    protected function determineGender(array $jobFor)
    {
    ...
    public function storeJobEmail($data)
    {
        ...
        if (empty($data['user_email_job_id'])) {
            return ['status' => 'fail', 'message' => 'Job ID is required'];
        }

        // Retrieve the job or fail with an exception if not found
        ...
        // Update job details based on input or default to user's metadata
        ...
        // Fire an event after the job creation
    ...
    public function jobToData($job)
    {
        ...
        $data = [
            ...
            'due_date' => Carbon::parse($job->due)->toDateString(),
            'due_time' => Carbon::parse($job->due)->toTimeString(),
            'job_for' => $this->translateJobFor($job)
        ];

        return $data;
    }

    protected function translateJobFor($job)
    {
        $labels = [];
